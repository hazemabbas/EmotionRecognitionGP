...
for i in range(0,10):
    ...
    print("training Adaboost %s" %i) #train SVM
    ...
    bdt_real.fit(npar_train, training_labels)
	#bdt_discrete.fit(npar_train, training_labels)


    print("getting accuracies %s" %i) #Use score() function to get accuracy
    npar_pred = np.array(prediction_data)
    pred_lin = bdt_real.score(npar_pred, prediction_labels)
    print ("Adaboost : ", pred_lin)
    accur_lin.append(pred_lin) #Store accuracy in a list

print("Mean value Adaboost: %s" %np.mean(accur_lin))
